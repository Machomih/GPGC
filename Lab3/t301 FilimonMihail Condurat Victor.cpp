...
public:
    ...
    int isIn(CComplex &x, int &iterred) {
        int rez = 0;
        CComplex z0, z1;
        z0 = CComplex(0, 0);
        for (iterred = 1; iterred < m.nriter; iterred++) {
            z1 = z0 * z0 + x;
    ...
    void display(double xmin, double ymin, double xmax, double ymax) {
        ...
        for (double x = xmin; x <= xmax; x += RX_JF)
            for (double y = ymin; y <= ymax; y += RY_JF) {
                iterred = 0;
                CComplex z(x, y);
                int r = isIn(z, iterred);
                if (r == 0) {
                    glColor3f(1.0, 0.1, 0.1); // rosu
                } else {
                    glColor3f(0.07 * iterred, 0.1, 0.1); // nuante de rosu
                }
